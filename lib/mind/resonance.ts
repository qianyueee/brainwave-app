...
/**
 * 入力レンジ。下限は 1Hz（スペクトルの最初のビン）、上限は 45Hz。
 *
 * かつてはスペクトルの上限をそのまま使っていたが、その上限は商用電源のハムを
 * **見るため**に広げた診断用の値であって、誘導に使える帯域ではない。45Hz より
 * 上は脳波としては読めず、50/60Hz は電源そのもの——そこを指定できるように
 * しても、乗れたかどうかを読む材料にはならない。
 */
export const TARGET_HZ_MIN = 1;
export const TARGET_HZ_MAX = 45;
