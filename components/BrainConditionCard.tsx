 ...
 * ホームの脳コンディション統合カード。
 *
 * 上段の 3 値（Rate/Clarity/Reset）は**脳波測定からの自動算出**、下段のスライ
 * ダーは**ユーザーが毎日入れる自己評価**——出どころの違う 2 種類が 1 枚に同居
 * するので、上段は溝（インセット）に沈めて「読むもの」、下段は面の上に置いて
 * 「触るもの」と手触りで分けている。
 *
...
export default function BrainConditionCard() {
  ...
  return (
    <div className="bg-surface border border-surface-border rounded-3xl p-5 flex flex-col gap-4 neu-raised">
      <div className="flex items-center justify-between gap-2">
        <p className="text-sm text-text-secondary">脳コンディション</p>
        <Link href="/brain" className="text-xs text-primary font-medium">
          測定へ →
        </Link>
      </div>

      {/* 測定値 — 溝に沈めた 3 値。区切り線は最後の1つを除いて引く */}
      <div className="rounded-2xl bg-navy neu-inset grid grid-cols-3 py-3.5">
        {metrics.map((m, i) => (
          <div
            key={m.key}
            title={m.fullName}
