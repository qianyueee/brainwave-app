...
| `rawPerSec` | 前の帯域パケットからこの行までに届いた raw 波形サンプル数 |
| `spectrum` | その秒の 1〜45Hz 毎の FFT 振幅を `;` 区切りで並べたもの（先頭が 1Hz） |

`rawPerSec` と `spectrum` は**スペクトルがおかしいときに後から原因を切り分ける
ための列**です。`thinkgear.py` の `_spectrum` は窓を「時間」ではなく
「サンプル数」で切り、その 512 個がちょうど 1 秒だと仮定して周波数軸を作ります。
raw が取りこぼされると 512 個が 1 秒より長い時間に広がり、**軸全体が
`512 / rawPerSec` 倍に伸びます**（実測: バイト欠落 0.2% で 10Hz の α が 11Hz に、
2% で 15Hz に化けます）。つまり:

- `rawPerSec` が 512 前後 … その行のスペクトルの Hz 軸は信用してよい
- `rawPerSec` が明らかに小さい … 脳が変わったのではなく**軸が伸びている**

Bluetooth や USB シリアルの状態で変わるので、まず測定中ずっと 512 付近に
張り付いているかを確認してください。なお起動直後（と再同期直後）の 1 行だけは
1 秒に満たない区間を数えるため小さく出ます。

`spectrum` があるぶん CSV は従来の約 10 倍（15 分の測定で 0.5MB 程度）になります。
...
| 周波数スペクトルの形がおかしい（山が変な Hz に立つ・1/f の減衰にならない） | CSV の `rawPerSec` 列を見る。512 付近でなければ raw の取りこぼしで Hz 軸が伸びている（上記「CSV ログの列」参照）。ヘッドセットとの距離・他の Bluetooth 機器・PC の負荷を確認 |
